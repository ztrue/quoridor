 ...
 * Quoridor game
 */
angular.module('quoridor.game', ['comet'])
    ...
    link: function(scope) {
      ...
      scope.init = function() {
        comet
          .connect()
          // load state
          .emit(scope.Events.client.GET_STATE, null, function(data) {
            if (data.success) {
       ...
       */
      scope.join = function() {
        comet.emit(scope.Events.client.JOIN, null, function(data) {
          if (data.success) {
       ...
       */
      scope.exit = function() {
        comet.emit(scope.Events.client.EXIT, null, function(data) {
          if (data.success) {
            scope.model.uid = null;
          }
        });
